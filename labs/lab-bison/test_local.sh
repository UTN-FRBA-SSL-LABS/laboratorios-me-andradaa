...
check E1.1 "parser1.y compila" 5 \
  bash -c 'cd ejercicio1 && bison -d parser1.y && flex scanner1.l && gcc lex.yy.c parser1.tab.c -o calc1.exe'

check E1.2 "Suma correctamente (3 + 4 = 7)" 5 \
  bash -c "cd ejercicio1 && echo '3 + 4' | ./calc1.exe | grep -q '= 7'"

check E1.3 "Resta correctamente (10 - 3 = 7)" 5 \
  bash -c "cd ejercicio1 && echo '10 - 3' | ./calc1.exe | grep -q '= 7'"

check E1.4 "Multiplicacion correctamente (2 * 5 = 10)" 5 \
  bash -c "cd ejercicio1 && echo '2 * 5' | ./calc1.exe | grep -q '= 10'"

check E1.5 "Division correctamente (8 / 2 = 4)" 5 \
  bash -c "cd ejercicio1 && echo '8 / 2' | ./calc1.exe | grep -q '= 4'"

# ── Ejercicio 2 ────────────────────────────────────
check E2.1 "parser2.y compila" 5 \
  bash -c 'cd ejercicio2 && bison -d parser2.y && flex scanner2.l && gcc lex.yy.c parser2.tab.c -o calc2.exe'
...
check E2.4 "Imprime numero correctamente" 5 \
  bash -c "cd ejercicio2 && echo '42' | ./calc2.exe | grep -q 'Numero: 42'"

check E2.5 "Imprime identificador correctamente" 5 \
  bash -c "cd ejercicio2 && echo 'hola' | ./calc2.exe | grep -q 'Identificador: hola'"

# ── Ejercicio 3 ────────────────────────────────────
check E3.1 "parser3.y compila" 5 \
  bash -c 'cd ejercicio3 && bison -d parser3.y && flex scanner3.l && gcc lex.yy.c parser3.tab.c -o calc3.exe -lm'
...
check E4.1 "parser4.y compila" 5 \
  bash -c 'cd ejercicio4 && bison -d parser4.y && flex scanner4.l && gcc lex.yy.c parser4.tab.c -o calc4.exe'

check E4.2 "Procesa expresion valida antes del error (= 7)" 5 \
  bash -c "cd ejercicio4 && ./calc4.exe < entrada.txt | grep -q '= 7'"

check E4.3 "Continua tras error y procesa expresion valida (= 10)" 5 \
  bash -c "cd ejercicio4 && ./calc4.exe < entrada.txt | grep -q '= 10'"
